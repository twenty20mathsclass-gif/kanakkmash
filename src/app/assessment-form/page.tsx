  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (firestore) {
        ...
        await addDoc(collection(firestore, "pre_assessment"), submissionData);
      }

      sessionStorage.setItem("assessmentUser", JSON.stringify(formData));
      await new Promise((r) => setTimeout(r, 800));
      router.push(
        `/assessment-test${invoiceId ? `?invoiceId=${invoiceId}` : ""}`,
      );
    } catch (err: any) {
      console.warn("Failed to save assessment registration:", err);
      setErrors({
        form: "Could not register details. Please check your connection and try again.",
      });
      setLoading(false);
    }
  };
        ...
        <div className="bg-card border border-border rounded-3xl p-8 shadow-lg">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm">
              Fill in your details to begin the test
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <div>
              ...
              {errors.class && (
                <p className="text-destructive text-xs mt-1">{errors.class}</p>
              )}
            </div>
