...
import { Loader2, ArrowLeft, Image as ImageIcon, Plus, Trash2, Edit, LayoutGrid, CheckCircle2, BookOpen, BarChart, Clock } from 'lucide-react';
import { Reveal } from '@/components/shared/reveal';

export default function AdminEditAssessmentQuestionPage() {
        ...
        const fetchQuestion = async () => {
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    toast({ variant: 'destructive', title: 'Lookup Failed', description: 'This question record could not be found.' });
                    router.back();
                }
            } catch (error) {
                console.error("Error fetching question:", error);
                toast({ variant: 'destructive', title: 'System Error', description: 'Failed to retrieve question data.' });
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!question.trim()) {
            toast({ variant: 'destructive', title: 'Error', description: 'Narrative content is required.' });
            return;
        }

        if (options.some(opt => !opt.trim())) {
            toast({ variant: 'destructive', title: 'Error', description: 'All options must be defined.' });
            return;
        }
        ...
        try {
            ...
            toast({ title: 'Question Synchronized', description: 'Database updated successfully.' });
            router.push(`/admin/assessment/${encodeURIComponent(className)}`);
        } catch (error) {
            console.error("Error updating question:", error);
            toast({ variant: 'destructive', title: 'Action Failed', description: 'Failed to commit changes to database.' });
        } finally {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-40 gap-6">
                ...
                <p className="font-black text-slate-300 uppercase tracking-widest text-xs">Fetching configuration...</p>
            </div>
        );
    }
                ...
                <div className="flex items-center gap-8">
                    ...
                    <div>
                        <div className="flex items-center gap-2 text-primary font-bold text-[11px] uppercase tracking-widest mb-1.5">
                            <LayoutGrid size={14} /> Global Bank Moderator
                        </div>
                        <h1 className="text-4xl font-black font-headline tracking-tight">Edit Assessment Item</h1>
                        <p className="text-muted-foreground text-lg mt-1">Modifying repository entry for <span className="font-bold text-slate-800">{className}</span>.</p>
                    </div>
                </div>
            </Reveal>
                ...
                <Card className="border-none shadow-2xl rounded-[3rem] overflow-hidden bg-white ring-1 ring-gray-100">
                    <CardHeader className="bg-slate-50/50 p-10 border-b border-dashed">
                        <CardTitle className="text-2xl font-black font-headline tracking-tighter flex items-center gap-3">
                           <Edit className="h-6 w-6 text-primary" /> Update Configuration
                        </CardTitle>
                        <CardDescription className="text-lg font-medium">Refine question narrative and valid response paths.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-10">
                        <form onSubmit={handleSubmit} className="space-y-12">
                            ...
                            <div className="space-y-6">
                                ...
                                <Textarea 
                                    id="question" 
                                    placeholder="Enter the question narrative here..." 
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    {imageUrl && (
                                         <div className="mt-4 relative h-48 w-full rounded-3xl border-2 border-dashed border-slate-100 overflow-hidden bg-slate-50 flex items-center justify-center">
                                             <img src={imageUrl} alt="Asset Preview" className="h-full object-contain p-4" />
                                         </div>
                                    )}
                                </div>
                             </div>
                                                ...
                                                <Input 
                                                    ...
                                                    className={`h-16 pl-6 pr-24 rounded-2xl text-lg font-bold transition-all border-2 ${
                                                        String(correctIndex) === String(idx) 
                                                            ...
                                                            : 'bg-slate-50/50 border-slate-50 hover:border-slate-100 placeholder:text-slate-300'
                                                    }`}
                                                    required
                                                />
                                                {String(correctIndex) === String(idx) && (
                            ...
                            <div className="flex gap-6 pt-12 border-t border-dashed">
                                ...
                                <Button 
                                    ...
                                    className="flex-[2] h-16 text-lg font-black rounded-2xl shadow-2xl shadow-primary/30 transition-all active:scale-95 bg-slate-900 hover:bg-slate-800"
                                >
                                    {isSubmitting ? (
                                        ...
                                        <div className="flex items-center gap-3">
                                            Apply Database Updates <Plus className="h-5 w-5" />
                                        </div>
                                    )}
                                </Button>
                            </div>
